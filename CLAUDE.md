...
At each gating step, a token key `"layer:pos"` is FNV-1a hashed into a `Slot4096`, projected onto a lattice coordinate, the slot's mantissa words are decomposed into `charge/entropy/tension` fields, XOR'd with the layer's history to produce a `feedback` value, and that feedback selects an expert index. The history is updated, so the next token's routing is influenced by this one's outcome.
...
*HDGL-28 overhead is the cost of `route_token_recursive` per layer per token — approximately 60 × FNV hash + lattice projection + history update — negligible against the 2.4ms SSD I/O per layer.*
...
```
           → CPU: softmax(gate_scores)
                  route_token_recursive(layer:pos, &history[layer])
...
Per-token routing:
  Token key  = FNV-1a("{layer}:{pos}")
  Slot coord = mantissa_words[0] % total_slots
  feedback   = slot.charge ^ slot.entropy ^ slot.tension
  combined   = feedback ^ H.last_feedback ^ H.last_expert_id
  expert_id  = (slot_coord + combined) % NUM_EXPERTS
  H updated  → next token's routing is history-conditioned
